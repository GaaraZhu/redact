...
| Command | Purpose |
|---|---|
| `gate init [--harness claude-code\|opencode\|copilot-cli] [--scope global\|project]` | Register the hook in the agent harness. `claude-code` (default) writes `~/.claude/settings.json`; `opencode` writes a TypeScript plugin at the chosen scope; `copilot-cli` writes `.github/hooks/PreToolUse.json` in the current repo root. |
